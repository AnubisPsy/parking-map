...
import { Link2, Upload, Plus, ParkingSquare, Trash2, List, CalendarDays, X, ChevronDown, MapPin, BarChart2, Info, LayoutGrid } from 'lucide-react';
...
function buildDateUnitGroups(parkings, unitColors) {
  ...
  parkings.forEach(p => {
    ...
    if (!dg.subgroups.has(uk))
      dg.subgroups.set(uk, { key: uk, dateKey: dk, unitId: p.unit_id, label: p.unit_name ? `${p.unit_name} (${p.unit_id})` : p.unit_id, color: unitColors[p.unit_id] || '#64748b', items: [], count: 0 });
...
export default function Home() {
  ...
  useEffect(() => { fetchParkings().then(setParkings).catch(e => setError(e.message)).finally(() => setLoading(false)); }, []);
  ...
  const groupFilteredParkings = useMemo(() => {
    ...
    if (groupBy === 'date_unit') {
      if (filterKey.includes('||')) {
        ...
        // Date group selected
        return parkings.filter(p => (dateKey(p) || 'sin-fecha') === filterKey);
      }
    }
  ...
  function handleCreated(parking) { setParkings(prev => [parking, ...prev]); setHighlightId(parking.id); }
  function handleImported(newParkings) { setParkings(prev => [...newParkings, ...prev]); if (newParkings.length > 0) setHighlightId(newParkings[0].id); }

  async function handleDeleteGroup(e, group) {
    e.stopPropagation();
    const ok = await confirm({ title: `¿Eliminar "${group.label}"?`, message: `Se eliminarán los ${group.count} registro${group.count !== 1 ? 's' : ''}. Esta acción no se puede deshacer.` });
    if (!ok) return;
    try {
  ...
  async function handleDeleteAll() {
    const ok = await confirm({ title: '¿Eliminar todos los registros?', message: `Se eliminarán los ${parkings.length} registro${parkings.length !== 1 ? 's' : ''} de todas las unidades. Esta acción no se puede deshacer.` });
    if (!ok) return;
    try { await deleteAllParkings(); setParkings([]); setFilterKey('all'); setExpandedGroups(new Set()); setExpandedSubgroups(new Set()); setHighlightId(null); }
    catch (err) { console.error(err); }
  }

  async function handleDeleteOne(e, id) {
    e.stopPropagation();
    const ok = await confirm({ title: '¿Eliminar este registro?', message: 'Esta acción no se puede deshacer.' });
    if (!ok) return;
    try { await deleteParking(id); setParkings(prev => prev.filter(p => p.id !== id)); if (highlightId === id) setHighlightId(null); }
    catch (err) { console.error(err); }
  }

  function handleSelect(id) { setHighlightId(prev => prev === id ? null : id); }
  ...
  function handleInfoLeave() { setTooltipParking(null); }
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 4 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3, flexWrap: 'wrap' }}>
              ...
              {showUnit && (
                <span style={{ fontSize: 10, fontWeight: 700, color: 'white', background: itemColor, padding: '1px 6px', borderRadius: 20 }}>
                  {p.unit_name || p.unit_id}
                </span>
              )}
              {duration && <span style={{ fontSize: 11, fontWeight: 600, color: isHighlighted ? '#2563eb' : '#374151' }}>{duration}</span>}
              {isHighlighted && <span style={{ marginLeft: 'auto', fontSize: 10, color: '#2563eb', fontWeight: 700 }}>● en mapa</span>}
            </div>
            {showDate && start && <p style={{ fontSize: 11, color: '#64748b', marginBottom: 2 }}>{start}</p>}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 2, flexShrink: 0 }}>
            <span onMouseEnter={(e) => handleInfoEnter(e, p, itemColor)} onMouseLeave={handleInfoLeave}
              style={{ color: '#cbd5e1', display: 'flex', alignItems: 'center', padding: '2px 4px', borderRadius: 4, cursor: 'default' }}>
              <Info size={12} />
            </span>
            <button onClick={(e) => handleDeleteOne(e, p.id)}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: '#93c5fd' }}>
              {parkings.length} registro{parkings.length !== 1 ? 's' : ''}{' · '}{groups.length} grupo{groups.length !== 1 ? 's' : ''}
            </p>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => setShowShare(true)} style={{ ...btnOutline, display: 'flex', alignItems: 'center', gap: 6 }} disabled={parkings.length === 0}><Link2 size={14} /> Compartir mapa</button>
          <button onClick={() => setShowSharesList(true)} style={{ ...btnOutline, display: 'flex', alignItems: 'center', gap: 6 }}><List size={14} /> Mis enlaces</button>
          <button onClick={() => navigate('/reports')} style={{ ...btnOutline, display: 'flex', alignItems: 'center', gap: 6 }}><BarChart2 size={14} /> Reportes</button>
          <button onClick={() => setShowImport(true)} style={{ ...btnOutline, display: 'flex', alignItems: 'center', gap: 6 }}><Upload size={14} /> Importar</button>
          <button onClick={() => setShowForm(true)} style={{ ...btnWhite, display: 'flex', alignItems: 'center', gap: 6 }}><Plus size={14} /> Nuevo registro</button>
        </div>
      </header>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        <aside style={sidebar}>
          ...
          <div style={{ padding: '10px 14px 8px', borderBottom: '1px solid #e2e8f0', flexShrink: 0 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              ...
              {parkings.length > 0 && (
                <button onClick={handleDeleteAll} style={{ background: 'transparent', border: '1px solid #e2e8f0', borderRadius: 6, cursor: 'pointer', color: '#94a3b8', display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, padding: '3px 8px', fontFamily: 'inherit' }} title="Borrar todos los registros">
            ...
            <div style={{ padding: '8px 16px 10px', borderBottom: '1px solid #e2e8f0', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                {(filterDateFrom || filterDateTo) && (
                  <button onClick={() => { setFilterDateFrom(''); setFilterDateTo(''); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', display: 'flex', padding: 2, flexShrink: 0, borderRadius: 4 }} title="Quitar filtro"><X size={14} /></button>
                )}
              </div>
              {(filterDateFrom || filterDateTo) && (
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px', display: 'flex', flexDirection: 'column', gap: 6 }}>
            {loading && <div style={{ textAlign: 'center', padding: 40, color: '#94a3b8', fontSize: 13 }}>Cargando registros...</div>}
            {error && <div style={{ background: '#fef2f2', color: '#dc2626', padding: 16, borderRadius: 8, fontSize: 13 }}>Error: {error}</div>}
                  ...
                  return (
                    <div key={g.key} style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                      <div style={{ display: 'flex', gap: 4, alignItems: 'stretch' }}>
                        <button style={{ ...(isActive ? unitBtnActive(g.color) : unitBtn), flex: 1 }} onClick={() => toggleGroup(g.key)}>
                        ...
                        {groupBy === 'unit' && (
                          <button onClick={(e) => handleDeleteGroup(e, g)} style={{ padding: '0 10px', background: '#fef2f2', border: '1px solid #fee2e2', borderRadius: 8, cursor: 'pointer', color: '#dc2626', display: 'flex', alignItems: 'center', flexShrink: 0 }} title={`Eliminar registros de ${g.label}`}>
                            <Trash2 size={12} />
                          </button>
                        )}
                      </div>
                      {isExpanded && (
                        <div style={{ paddingLeft: 14, display: 'flex', flexDirection: 'column', gap: 4 }}>
                          {items.length === 0
                            ...
                            : items.map((p, idx) => renderItem(p, idx, unitColors[p.unit_id] || '#64748b', groupBy !== 'unit', groupBy !== 'date', groupBy !== 'address'))
                          }
                        </div>
                      )}
                    </div>
                          ...
                          {filteredSubgroups.map(sg => {
                            ...
                            return (
                              <div key={sg.key} style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                                ...
                                <div style={{ display: 'flex', gap: 4, alignItems: 'stretch' }}>
                                  <button style={{ ...(sgActive ? unitBtnActive(sg.color) : unitBtn), flex: 1, fontSize: 12 }} onClick={() => toggleUnitSubgroup(sg.key, dg.key)}>
                                ...
                                {sgExpanded && (
                                  <div style={{ paddingLeft: 12, display: 'flex', flexDirection: 'column', gap: 3 }}>
                                    {sg.items.map((p, idx) => renderItem(p, idx, sg.color, false, true, true))}
                                  </div>
                                )}
                              </div>
                            );
        ...
        <main style={{ flex: 1, position: 'relative' }}>
          {!loading && <MapView parkings={visibleParkings} unitColors={unitColors} highlightId={highlightId} onMarkerClick={handleSelect} />}
          {loading && <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#94a3b8' }}>Cargando mapa...</div>}
        </main>
      </div>

      {showForm && <ParkingForm onCreated={handleCreated} onClose={() => setShowForm(false)} />}
      {showShare && <ShareLinkModal units={buildGroups(parkings, 'unit', unitColors).map(g => ({ id: g.unitId, name: g.label, count: g.count }))} onClose={() => setShowShare(false)} />}
      {showImport && <ImportModal onImported={handleImported} onClose={() => setShowImport(false)} />}
      {showSharesList && <SharesListModal onClose={() => setShowSharesList(false)} />}
      {dialog}

      {tooltipParking && (
        <div style={{ position: 'fixed', left: 368, top: tooltipPos, width: 262, zIndex: 3000, pointerEvents: 'none', borderRadius: 12, overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,0.22)', border: '1px solid rgba(0,0,0,0.07)' }}>
          <div style={{ background: tooltipParking._color, color: 'white', padding: '10px 14px', fontWeight: 700, fontSize: 14 }}>{tooltipParking.unit_name || tooltipParking.unit_id}</div>
          <div style={{ background: 'white', padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 2 }}>Registrado: {fmtDt(tooltipParking.created_at)}</div>
          </div>
        </div>
      )}
    </div>
...
const sidebar = { width: 360, background: 'white', borderRight: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', overflow: 'hidden', flexShrink: 0 };
const btnWhite = { padding: '8px 16px', background: 'white', color: '#1d4ed8', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer' };
const btnOutline = { padding: '8px 16px', background: 'transparent', color: 'white', border: '1px solid rgba(255,255,255,0.4)', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' };
